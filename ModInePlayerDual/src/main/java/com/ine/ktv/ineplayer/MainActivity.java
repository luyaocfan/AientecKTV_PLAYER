...
public class MainActivity extends AppCompatActivity implements InePlayerController.EventListen, VODPlayList.EventListener, IneSocketClient.EventListener {
    ...
    static final int MaxCacheCount = 4;
    static final int MaxCacheSize = 1024*1024*16;  // for start tup
    static final int [] CacheBandwidthKBS = new int[] {1024, 256, 128, 64};
    static final int PlayingBufferSize = MaxCacheSize*2; // for current playing
...
//            "http://192.168.99.152/63029ZH1W.mp4",
...
//            "http://106.104.151.145:10003/mtv/4K1.mp4",
        ...
        btnOrderSong.setOnClickListener( (sender) -> {
            for(int i=0; i<PlayList.length; i++) {
                if(PlayList[i].startsWith("http"))
                    controller.AddOrderSong(PlayList[i], "" + (orderSerialNo++) + ". " + new File(PlayList[i]).getName().split("\\.")[0], PlayingBufferSize);
                else
                    controller.AddOrderSong(PlayList[i], "" + (orderSerialNo++) + ". " + new File(PlayList[i]).getName().split("\\.")[0], PlayingBufferSize);
            }
        });
    ...
    public void AddSong(String songId, boolean bInsert) {
        ...
        if(bInsert && !controller.isInPublicVideo())
                controller.InsertOrderSong(1, songId , ""+(orderSerialNo++)+". "+songId, PlayingBufferSize);
        controller.AddOrderSong(songId, ""+(orderSerialNo++)+". "+songId, PlayingBufferSize);
    }
...
//VODPlayList.EventListen
    ...
    public void onOrderSongData(VODPlayList sender, VODPlayList.OrderPlayItem[] newItems) {
        ...
        for(int i=0; i<newItems.length; i++) {
            ...
            controller.AddOrderSong(playItem.getSongFile(), ""+ (playItem.getId())+". "+playItem.getSongName(), PlayingBufferSize);
        }
    }
    @Override
    ...
    void DemoAmbLight() {
        ...
        if(playerView.getVisibility()==View.VISIBLE) {
            ...
            try {
                PixelCopy.request(playerView, smallCaptureBitmap, listener, mainHandler);
            }
            catch (Exception ex) {
